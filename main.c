...
int LED_BLINK_TIMER_PERIOD = 9999;
...
/* thermistor's temperature measurement  */
...

{
    ...
    /* ADC init failed. Stop program execution */
    if (result != CY_RSLT_SUCCESS)
    {
        CY_ASSERT(0);
    }
    ...
    /* NTC GPIO init failed. Stop program execution */
    if (result != CY_RSLT_SUCCESS)
    {
        CY_ASSERT(0);
    }

    // receiver buffer init
    count = 0;
    ...
    for (;;)
    {
             ...
             == CY_RSLT_SUCCESS)
        {
            if (uart_read_value == '\r')
            {
            	...
            	//if neither of previous two
            	buf [6] = 0;
            	if (!strcmp (buf, "period")){
            		int period = atoi (&buf[7]);
            		if (period > 5) period = 5;
            		if (period < 1) period = 1;
                    printf("new period %d sec.\r\n", period);
            		period *= 10000;
            		period -= 1;
                	LED_BLINK_TIMER_PERIOD = period;
                	/* reinitialize timer to toggle the LED */
                	cyhal_timer_stop(&led_blink_timer);
                	timer_init();
                	if ( led_blink_active_flag ) cyhal_timer_start(&led_blink_timer);
                	else cyhal_timer_stop(&led_blink_timer);
            	}

				//receiver buffer reinitialize
				count = 0;
				memset (buf, 0, sizeof buf);
				...
				if (count < sizeof buf){
					if ( uart_read_value != '\n') buf [count++] = uart_read_value;
				}else{
					// get rid of trash - receiver buffer reinitialize
					count = 0;
					memset (buf, 0, sizeof buf);
				}
			}
        ...
        if (timer_interrupt_flag)
        {
            ...
            float T_value = mtb_thermistor_ntc_gpio_get_temp(&mtb_thermistor_obj);
            printf("%+2.1f\r\n", T_value);
        }
    }
}
